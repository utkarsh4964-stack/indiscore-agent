...
def run_assessment(upi_data, bill_data, api_key):
    ...
    # Force the key into the environment context where CrewAI's background processes can look it up
    os.environ["GROQ_API_KEY"] = active_key

    # Specify the target model configuration directly via native string tracking.
    # This prevents Pydantic from blowing up on third-party object types.
    target_model = "groq/llama-3.3-70b-specdec" 
