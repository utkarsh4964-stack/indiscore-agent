...
3-agent sequential pipeline using the Groq SDK directly.
...
def _get_api_key(api_key: str = "") -> str:
    """Resolve API key: argument > Streamlit secrets > environment variable."""
...
def run_assessment(upi_data: str, bill_data: str, api_key: str = "") -> str:
    """
    Sequential 3-agent credit assessment pipeline.
    ...
    if not key:
        return (
            ...
            "Add it to `.streamlit/secrets.toml` as `GROQ_API_KEY = \"gsk_...\"` "
...
Analyse raw UPI transaction logs and bank statements to:
...
- Signs of financial distress or misrepresentation
Rate each risk factor clearly.""",
...
Synthesise financial stability and risk reports to produce a final credit score between 300 and 900.
...
Your report must be professional, transparent, and end with the EXACT line: FINAL_SCORE: [number]""",
        user_message=f"""Generate the final credit underwriting report.
...
(Key income, spending, and savings insights with specific numbers)

## 🚨 Risk & Fraud Assessment
(Risk level, red flags detected, positive signals)

## 🗺️ Improvement Roadmap
(3–5 concrete, actionable steps to improve the credit score)

## 📌 Underwriter Decision
(Final lending recommendation with clear rationale)
