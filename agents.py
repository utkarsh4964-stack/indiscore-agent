...
def run_assessment(upi_data, bill_data, api_key):
    # Enforce active credential routing from the user UI or environment settings
    ...
    # Using CrewAI's explicit native LLM class wrapper.
    # Specifying the provider prefix explicitly inside the native wrapper block
    # handles internal data types correctly and silences Pydantic v2.
    my_llm = LLM(
    ...
    tx_agent = Agent(
        ...
        goal='Extract clear income and spending metrics from unstructured text inputs.',
    ...
    underwriter = Agent(
        ...
        goal='Synthesize raw auditor summaries into an unified credit safety evaluation profile scoring between 300 and 900.',
